...
import com.application.coopcycle.service.CooperativeQueryService;
...
/**
 * REST controller for managing {@link com.application.coopcycle.domain.Cooperative}.
 */
@RestController
@RequestMapping("/api")
public class CooperativeResource {
    ...
    public CooperativeResource(
        CooperativeService cooperativeService,
        CooperativeRepository cooperativeRepository,
        CooperativeQueryService cooperativeQueryService
    ) {
    ...
    public ResponseEntity<List<CooperativeDTO>> getAllCooperatives(CooperativeCriteria criteria, Pageable pageable) {
        log.debug("REST request to get Cooperatives by criteria: {}", criteria);
        Page<CooperativeDTO> page = cooperativeQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /cooperatives/count} : count all the cooperatives.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/cooperatives/count")
    public ResponseEntity<Long> countCooperatives(CooperativeCriteria criteria) {
        log.debug("REST request to count Cooperatives by criteria: {}", criteria);
        return ResponseEntity.ok().body(cooperativeQueryService.countByCriteria(criteria));
    }
